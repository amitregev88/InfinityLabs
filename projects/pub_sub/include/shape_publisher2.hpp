...
public:
    ...
    void Init(size_t port_, std::string serverip_);
    void Publish();
...
void ShapePublisher::Init(size_t port_, std::string serverip_)
{
    m_port = port_;
    ...
    memset(&client_addr[1], 0, sizeof(client_addr[1]));
    ...
    int res = bind(fd, (const struct sockaddr *)&m_server_addr, sizeof(m_server_addr));
    assert(res == 0);
    ...
    {
        char buffer[200] = {"\0"};
        int num_of_bytes = recvfrom(m_fd, (char *)buffer, 200, MSG_WAITALL, (struct sockaddr *)&client_addr, &len);
        assert(num_of_bytes > 0);
        Parse(std::stoi(buffer), client_addr[i]);
        ++i;
    }
...
void ShapePublisher::Publish()
{
    ...
    for (size_t idx = 0; m_subs.size() > idx; ++idx)
    {
        ...
        while (2 > count)
        {
            ...
            {
                ++count;
                sendto(m_fd, msg.c_str(), msg.size(), 0, (const sockaddr *)&tmp_struct, sizeof(m_subs[idx].GetStruct()));
                sleep(3);
            }
