...
void ShapePublisher::Init()
{
    ...
    int res = bind(m_fd, (struct sockaddr *)&m_localSock, sizeof(m_localSock));
    assert(res >= 0);
...
void ShapePublisher::Publish()
{
    ...
    {
        //lock -not sure if neccery
        //TODO check if sub sent to thread or not -addd flag is_send_to_thread = init to false
        //if flaf is on or not
