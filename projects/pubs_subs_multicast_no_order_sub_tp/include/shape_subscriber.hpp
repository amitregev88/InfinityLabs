...
private:
    ...
    static void Listen(int fd,  struct sockaddr_in group_addr);
};
...
// The Second digit to the left (if it register request): 1 - squarem , 2 - circle , 3 - both
    ...
    ThreadPool tp(2);
    tp.AddTask(std::bind(Listen, m_sockfd, m_group_addr));
...
void ShapeSubscriber::Listen(int fd,  struct sockaddr_in group_addr)
{
    while (true)
    {
        char buffer[200] = {"\0"};
        ...
        while(n_bytes_recived <= 0 )
        {
            ...
            n_bytes_recived = recvfrom(fd, (char *)buffer , 200, MSG_WAITALL, (struct sockaddr *)&group_addr, (socklen_t *)&len);
